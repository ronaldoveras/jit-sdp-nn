...
class IterativeForest(Scikit):
    ...
    def __init__(self, steps, classifier, features, target, soft_target, fading_factor, n_trees, n_updates=1):
        super().__init__(steps=steps, classifier=classifier, features=features, target=target,
                         soft_target=soft_target, fading_factor=fading_factor, batch_size=None)
    ...
    def train_iteration(self, inputs, targets):
        ...
        for i in range(self.n_updates - 1):
            permuted_indices = np.random.permutation(len(inputs))
            self.estimators[last_estimator].fit(
                inputs[permuted_indices], targets[permuted_indices])
